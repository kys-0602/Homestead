...
- 상점의 `SILVER 125 GOLD 150` 안내를 제거하고, 상점·데일리 퀘스트 표지판 근처에
  각각 `E MARKET`, `E DAILY REQUEST` 안내를 추가했다. 실제 키보드 상호작용과 같은 근접 판정을 사용한다.
- Debug 빌드와 CTest 29개 전체를 통과했다. Release 및 최종 제출 패키지 빌드도 성공했다.
- Release EXE `271,872 bytes`, pak `366,132 bytes`, 대표 세이브 `175 bytes`, README `3,281 bytes`.
  전체 제출물은 `641,460 bytes`이며 제한까지 `833,100 bytes`가 남는다.
...
- 맵의 정적 `Farmable` 플래그와 전용 흙 바닥 타일을 추가하고, 농장 오른쪽 아래에 8x6 경작 가능 구역을 배치했다.
- 괭이는 비차단·비물 타일 중 `Farmable` 플래그가 있는 곳에서만 새 경작지를 만들 수 있다. 이미 경작된 타일의 물 주기·심기·성장·수확 규칙은 바꾸지 않았다.
- 저장 파일에는 여전히 동적 경작/물 상태만 기록하므로, 기존 저장에서 이미 경작된 다른 위치는 정상적으로 다시 불러와 계속 사용할 수 있다.
- 맵 형식 검증, 실제 맵 콘텐츠, 도구 상호작용 테스트에 경작 가능/불가능 토지 검사를 추가했다.

검증:

- Debug 빌드 성공 및 CTest 26개 전체 통과.
- Release 빌드 성공. 수동 확인 필요: 갈색 흙 구역에서는 괭이가 작동하고, 일반 잔디·길에서는 작동하지 않는지, 경작 후 연결 타일과 물 주기 표현이 정상인지 확인.
- Release EXE `96,768 bytes` (변화 없음), pak `366,028 bytes` (`+20 bytes`), 대표 save `172 bytes` (변화 없음).
- 실제 제출 합계 `462,968 bytes` (`+20 bytes`), 상한까지 `1,011,592 bytes` 남음.
- EXE SHA-256: `a992be6fae46f7574d535d0a72915506f9f05c2a124af76d10c590706e808606`
- pak SHA-256: `6a98356166492d2b422ed34f65e1d4a2c9dcd679f9530878985794ee0a29d4f2`
- 대표 save SHA-256: `bf43597428e9a88655dc6ac098a093880c37a159a59805e56795971a4b1757aa`

다음 작업: 작물 도감 구현 또는 사용자의 수동 검증 피드백 반영.

## 기존 경작지 정리 및 저장 초기화

구현:

- 이전 저장 파일의 경작/작물 델타 중 현재의 `Farmable` 토지가 아닌 항목은 불러올 때 적용하지 않도록 했다. 따라서 새 논밭 구역 밖에 남아 있던 기존 경작지와 작물은 즉시 사라지고, 이후 저장에서는 다시 기록되지 않는다.
- ESC 메뉴에 `RESET SAVE` 항목을 추가했다. 첫 실행 뒤 `RESET AGAIN`으로 바뀐 상태에서 한 번 더 실행해야 초기화가 진행된다.
- 초기화는 `%LOCALAPPDATA%\Homestead`의 주 저장, 백업, 임시 파일을 모두 삭제하고 현재 게임을 1일차 06:00, 시작 골드, 괭이와 물뿌리개만 가진 농장 시작 위치로 되돌린다.
- 저장 파일 삭제 뒤 `NotFound` 상태로 복귀하는 전용 `SaveSystem` 자동 테스트를 추가했다.

검증:

- Debug 및 Release 빌드 성공, CTest 27개 전체 통과.
- 수동 확인 필요: 기존 논밭 밖 경작지가 저장을 불러온 직후 사라지는지, ESC → `RESET SAVE` → `RESET AGAIN` 뒤 시작 상태가 되고 게임을 다시 실행해도 초기 상태가 유지되는지 확인.
- Release EXE `98,816 bytes` (`+2,048 bytes`), pak `366,028 bytes` (변화 없음), 대표 save `172 bytes` (변화 없음).
- 실제 제출 합계 `465,016 bytes` (`+2,048 bytes`), 상한까지 `1,009,544 bytes` 남음.
- EXE SHA-256: `4445c1650a1ea12d67e1ed4d6625a681b14a8332f533af10f3decc80a085d60f`
- pak SHA-256: `6a98356166492d2b422ed34f65e1d4a2c9dcd679f9530878985794ee0a29d4f2`
- 대표 save SHA-256: `bf43597428e9a88655dc6ac098a093880c37a159a59805e56795971a4b1757aa`
...
- 시작 안내에서 더 이상 작동하지 않는 M키 상점 안내를 제거했다.
- 도구 선택/사용, 침대 수면, 상점·요청 표지판 상호작용을 현재 조작 방식에 맞게 짧은 세 줄의 안내로 갱신했다.

검증:

- Debug 및 Release 빌드 성공, CTest 27개 전체 통과.
- Release EXE `98,816 bytes`, pak `366,028 bytes`, 대표 save `172 bytes` (모두 변화 없음).
- 실제 제출 합계 `465,016 bytes`, 상한까지 `1,009,544 bytes` 남음.
- EXE SHA-256: `02063ad2291b059c1317a81306b25c5d41dfbc2f6e9e11fdf8e41a7ee9f34b7c`
- pak SHA-256: `6a98356166492d2b422ed34f65e1d4a2c9dcd679f9530878985794ee0a29d4f2`
- 대표 save SHA-256: `bf43597428e9a88655dc6ac098a093880c37a159a59805e56795971a4b1757aa`
...
- 수확물 인벤토리의 은색 품질에는 밝은 다이아 배지, 금색 품질에는 노란 별 배지를 추가했다.
- 기존의 작은 단색 사각형은 제거하고, 어두운 테두리 안에 형태가 다른 배지를 그려 작물 원래 색과 무관하게 구분되게 했다.
- 품질은 수확 순간 결정되는 값이므로, 성장 중인 월드 작물에는 임의의 품질 표시를 추가하지 않았다.

검증:

- Debug 및 Release 빌드 성공, CTest 27개 전체 통과.
- 수동 확인 필요: 은색·금색 수확물을 인벤토리에 함께 보관했을 때 각 배지의 모양과 겹침 없이 구분되는지 확인.
- Release EXE `99,328 bytes` (`+512 bytes`), pak `366,028 bytes` (변화 없음), 대표 save `172 bytes` (변화 없음).
- 실제 제출 합계 `465,528 bytes` (`+512 bytes`), 상한까지 `1,009,032 bytes` 남음.
- EXE SHA-256: `3aaee430ea8a983f8214c74ddead155bdf5bb2d4886c9c40295bb35e8ff02c23`
- pak SHA-256: `6a98356166492d2b422ed34f65e1d4a2c9dcd679f9530878985794ee0a29d4f2`
- 대표 save SHA-256: `bf43597428e9a88655dc6ac098a093880c37a159a59805e56795971a4b1757aa`
...
- 집 안 책장 근처에서 `E`를 누르면 작물 도감이 열리고, `E` 또는 `ESC`로 닫을 수 있다.
- 수확에 성공한 작물만 도감에서 해금한다. 해금된 행은 작물명·성장 일수·기본 판매가를, 미해금 행은 `UNSEEN`을 표시한다. 상단에는 은색 125%, 금색 150% 판매 보너스를 안내한다.
- `CropCatalogue`는 6개 작물을 1바이트 비트 집합으로 보관한다. save v6에 이 값을 기록하고 v1~v5 저장은 빈 도감으로 불러온다.
- 책장 근접 상호작용, 도감 비트 검증, v5 마이그레이션·알 수 없는 비트 거부 테스트를 추가했다.

검증:

- Debug 및 Release 빌드 성공, CTest 28개 전체 통과.
- 수동 확인 필요: 집 책장 근처에서 `E`로 도감을 열고 닫을 수 있는지, 수확 전에는 `UNSEEN`이고 수확 뒤 해당 작물 행의 정보가 표시되는지, 침대 저장 후 재실행해도 해금이 유지되는지 확인.
- Release EXE `100,352 bytes` (`+1,024 bytes`), pak `366,028 bytes` (변화 없음), 대표 save `173 bytes` (`+1 byte`).
- 실제 제출 합계 `466,553 bytes` (`+1,025 bytes`), 상한까지 `1,008,007 bytes` 남음.
- EXE SHA-256: `ceacb5a16d97633387aa2ad2d5f81c6306a9743bb29f2b47c33412cb45b8f034`
- pak SHA-256: `6a98356166492d2b422ed34f65e1d4a2c9dcd679f9530878985794ee0a29d4f2`
- 대표 save SHA-256: `000b746fdd583f36b048beb4061321e82497400389ca2a7e18bba3c79374c482`

다음 작업: 농장 배경 닭 구현 또는 사용자의 수동 검증 피드백 반영.

## 도감 품질 보너스 표기

구현:

- 도감의 품질 보너스를 `SILVER 125% GOLD 150%`로 명시했다.
- 선택된 비트맵 폰트에 없던 `%` 글리프를 원본 폰트에서 추가하고, 텍스트 렌더러와 pak 검증 목록에 등록했다.

검증:

- Debug 및 Release 빌드 성공, CTest 28개 전체 통과.
- Release EXE `100,352 bytes` (변화 없음), pak `366,048 bytes` (`+20 bytes`), 대표 save `173 bytes` (변화 없음).
- 실제 제출 합계 `466,573 bytes` (`+20 bytes`), 상한까지 `1,007,987 bytes` 남음.
- EXE SHA-256: `8440c8c1a477eed955486b875428013aadeb80992078357bb7c8e01a435e83ba`
- pak SHA-256: `00fa71d2d0da912b3dbfe2202c3f44684e9f0f3519bc5879959d97ab9766834b`
- 대표 save SHA-256: `000b746fdd583f36b048beb4061321e82497400389ca2a7e18bba3c79374c482`
...
- 농장 맵에서만 걷고 잠시 멈추는 닭 3마리와 개구리 1마리를 추가했다. 집 안에서는 갱신하거나 그리지 않는다.
- 닭은 플레이어·경작지·장식물과 충돌하거나 상호작용하지 않는 고정 경로 연출이며, 저장 데이터에 포함하지 않는다.
- `Chicken_01.png`, `Frog_01.png`에서 필요한 대기·이동 프레임만 패키징했다. 발 위치 Y 기준으로 플레이어와 자연스럽게 앞뒤 순서가 정렬된다.
- 고정 경로의 이동·정지·초기화 동작과 닭 스프라이트 패키징을 검증하는 테스트를 추가했다.
- 플레이어와 동물 사이에 작은 발밑 충돌 상자를 적용했다. 플레이어는 동물을 통과하지 않고, 동물은 플레이어와 겹치려 하면 잠시 정지한다.

검증:

- Debug 빌드 성공, CTest 29개 전체 통과.
- Release 빌드 성공, `HomesteadFarmAnimalsTests` 통과.
- 수동 확인 필요: 농장에 닭 3마리가 보이고, 걷기/정지 애니메이션 및 플레이어와의 앞뒤 순서가 자연스러운지 확인.
- Release EXE `103,936 bytes` (`+3,584 bytes`), pak `366,132 bytes` (`+84 bytes`), 대표 save `173 bytes` (변화 없음).
- 실제 제출 합계 `470,241 bytes` (`+3,668 bytes`), 상한까지 `1,004,319 bytes` 남음.
- EXE SHA-256: `b47a97d3b2a6291c148af957cfc49469a10ffe7a8d47748554e66c926e3d259a`
- pak SHA-256: `98c2a1f8a6b76566d9684f4dd1e95fe6046c3ebb68b4a1ec6e2cd61372947f78`
- representative save SHA-256: `000b746fdd583f36b048beb4061321e82497400389ca2a7e18bba3c79374c482`

다음 작업: 전체 게임 흐름 및 제출물 최종 수동 검증.
...
- 작물별로 은색과 금색 품질을 따로 기록하고, 도감 오른쪽에 `YES` 또는 `NO`로 표시한다.
- 세이브 v7에 작물 6종의 은색/금색 획득 여부를 12비트(2 byte)로 기록한다. v1~v6 세이브는 품질 기록 없음으로 그대로 불러온다.

검증:

- Debug 빌드 성공, CTest 29개 전체 통과.
- Release 제출 빌드 성공. 수동 검증: 은색/금색 작물을 각각 수확한 뒤 집 책장 도감에 표시되는지, 침대 저장 후 다시 불러왔을 때 유지되는지 확인 필요.
- Release EXE `104,960 bytes` (`+1,024 bytes`), pak `366,132 bytes` (변화 없음), 대표 save `175 bytes` (`+2 bytes`).
- 실제 제출 합계 `471,267 bytes` (`+1,026 bytes`), 상한까지 `1,003,293 bytes` 여유.
- EXE SHA-256: `e2214bff2d1764c4205c2c3db9f9a8b58cca287f8c1d3eb5e6d338c49190cefe`
- pak SHA-256: `98c2a1f8a6b76566d9684f4dd1e95fe6046c3ebb68b4a1ec6e2cd61372947f78`
- representative save SHA-256: `2b738db969fad7dd539bc39b0231bc90a51498583d86599c9de8048bea4c99c9`

다음 작업: 수동 게임 검증 또는 최종 제출 정리.

## 실행 파일 아이콘

구현:

- 농가·밀·일출 구성의 픽셀 아트 아이콘을 새로 제작하고, 16·16·32·32·48·48·64·64·128·128·256·256 해상도를 포함한 `resources/Homestead.ico`를 추가했다.
- Windows 리소스 스크립트로 실행 파일과 창 제목 표시줄에 같은 아이콘을 적용했다.

검증:

- CMake 재구성, Debug 빌드 성공, CTest 29개 전체 통과.
- Release 제출 빌드 성공. Windows 탐색기와 실제 창에서 아이콘 모양을 확인하는 수동 검증만 남음.
- Release EXE `271,360 bytes` (`+166,400 bytes`), pak `366,132 bytes` (변화 없음), 대표 save `175 bytes` (변화 없음).
...
- 32·32 닭 프레임의 실제 하단 픽셀은 발 기준점보다 9px 위에 있지만, 이전 충돌 상자는 투명 여백인 `-8~0`에 남아 있었다.
- 닭의 충돌 상자를 실제 몸통 하단에 맞춰 `-18~-10`으로 위로 옮겼다.
- 닭이 플레이어와 겹칠 때 멈추는 테스트와 플레이어와 닭 사이의 이동 충돌 테스트의 기준 위치를 새 상자에 맞춰 갱신했다.

검증:

- Debug 빌드 성공, CTest 29개 전체 통과.
- Release 제출 빌드 성공. EXE `271,360 bytes`, pak `366,132 bytes`, 대표 save `175 bytes`, README `3,281 bytes`.
- 실제 제출 합계 `640,948 bytes`, 상한까지 `833,612 bytes` 여유.
- EXE SHA-256: `ab2c452cb870383d9b2aa1934a1e8b7020f1be060d954c690d8a64fbb9b42655`

다음 작업: 닭 스프라이트와 동일한 높이에서 충돌하는지 수동 확인.
