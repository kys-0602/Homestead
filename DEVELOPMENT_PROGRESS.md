...
- 현재 기능 중심 농장 배치를 실제 플레이 동선에 맞게 개선
- farmhouse 외관과 출입구를 최종 농장 맵에 배치
- 작은 집 내부 맵과 농장↔집 전환 구현
- 집 내부 상호작용으로 즉시 저장하는 지점 추가
- save에 현재 맵 ID와 맵별 복귀 위치를 안정적인 ID로 기록
- 후속 맵 콘텐츠 검증 뒤 단계 17 `크기 최적화` 진행
