...
상세 기획과 구현 순서는 [DAILY_REQUESTS_PLAN.md](DAILY_REQUESTS_PLAN.md)를 따른다.

### 작업

- 날짜로부터 재현 가능한 하루 1개 작물 납품 의뢰를 생성한다.
- 농장 표지판 상호작용으로 키보드·마우스 지원 게시판 UI를 연다.
- inventory에서 요구 작물을 한 번에 납품하고 시장 판매보다 높은 골드 보상을 지급한다.
- 하루 중복 보상을 막고 다음 날 새 의뢰로 갱신한다.
- 완료 상태를 versioned save에 추가하고 이전 save를 안전하게 이관한다.
- 기존 침대 전용 저장 정책과 메뉴 입력 상호 배제를 유지한다.

### 완료 조건

- 생성·부족·성공·중복·overflow 규칙의 단위 테스트가 통과한다.
- save round-trip, 구버전 migration, malformed data 테스트가 통과한다.
- 표지판에서 의뢰 확인과 납품을 키보드와 마우스로 완료할 수 있다.
- 다음 날 새 의뢰가 표시되며 완료 상태가 침대 저장 후 복원된다.
- Debug/Release 빌드와 전체 테스트가 통과하고 제출 크기가 기록된다.
