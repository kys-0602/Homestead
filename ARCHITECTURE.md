...
- 집의 침대 앞에서 상호작용할 때만 하루를 종료한다. 60 tick 화면 전환의 중앙에서 날짜를 정확히 한 번 증가시키고 `DayChanged` 농사 갱신과 저장을 호출한다.
- 농장 구름 그림자는 충돌이나 저장 상태가 없는 월드 공간 렌더 효과이며, 플레이어와 무관하게 고정 tick으로 이동한다. 월드보다 앞이고 UI보다 뒤인 `Effect` layer에서 그리며 집 내부에서는 렌더링하지 않는다.
