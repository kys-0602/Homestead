...
constexpr std::uint16_t SaveVersion = 7;
...
bool ValidCrop(const CropInstance& crop) noexcept {
    ...
    if (!crop.active || definition == nullptr || crop.tileX < 0 || crop.tileX > 127 ||
        crop.tileY < 0 || crop.tileY > 127 || crop.wateredDays > definition->growthDays) return false;
    const std::uint8_t expectedStage = static_cast<std::uint8_t>(
        (static_cast<unsigned>(crop.wateredDays) * definition->finalStage) / definition->growthDays);
    return crop.stage == expectedStage;
}
...
bool EncodeSave(const SaveSnapshot& snapshot, std::vector<std::uint8_t>& bytes) noexcept {
    ...
    if (!CropCatalogue::IsValidBits(snapshot.discoveredCrops)) return false;
    if (!CropCatalogue::AreValidQualityBits(snapshot.discoveredCropQualities)) return false;
    U8(payload, snapshot.discoveredCrops);
    U16(payload, snapshot.discoveredCropQualities);
    ...
    for (const CropInstance& crop : snapshot.crops) {
        if (!ValidCrop(crop)) return false;
...
bool DecodeSave(const std::uint8_t* bytes, std::size_t size, SaveSnapshot& snapshot) noexcept {
    ...
    std::uint8_t requestCompleted = 0, discoveredCrops = 0;
    std::uint16_t discoveredCropQualities = 0;
    if (!reader.U32(x) || !reader.U32(y) || !reader.U16(result.day) || !reader.U16(result.minute) ||
        ...
        !(version < 6 || (reader.U8(discoveredCrops) && CropCatalogue::IsValidBits(discoveredCrops))) ||
        !(version < 7 || (reader.U16(discoveredCropQualities) &&
            CropCatalogue::AreValidQualityBits(discoveredCropQualities))) ||
    ...
    result.discoveredCrops = discoveredCrops;
    result.discoveredCropQualities = discoveredCropQualities;
