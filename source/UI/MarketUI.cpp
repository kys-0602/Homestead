...
bool AddMarketUI(std::uint16_t gold,std::uint8_t focus,const AssetStore& assets,RenderQueue& queue) noexcept {
    ...
    constexpr const char* labels[MarketItemCount]={"BUY WHEAT 4","BUY CARROT 6","BUY TOMATO 9",
        "SELL WHEAT 7","SELL CARROT 12","SELL TOMATO 20"};
    ...
    for(std::uint8_t i=0;i<MarketItemCount;++i)
        if(!AddBitmapText(labels[i],94,static_cast<float>(RowY+i*RowH+4),0xFFFFFFFFU,42,assets,queue))return false;
