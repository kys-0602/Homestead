...
std::vector<std::uint8_t> MakeFarm() {
    ...
    U16(bytes, 1); U16(bytes, 24); U16(bytes, 3); U16(bytes, 1);
    U16(bytes, 16); U16(bytes, 16); U16(bytes, 3); U16(bytes, 6); U32(bytes, 3);
...
int main() {
    ...
    std::uint16_t gold=Homestead::StartingGold;
    for(std::size_t crop=0;crop<Homestead::MarketCropCount;++crop)
        if(!Homestead::BuySeed(inventory,gold,crop))return 2;
    constexpr Homestead::ItemId seeds[]{Homestead::ItemId::WheatSeed,
        Homestead::ItemId::CarrotSeed,Homestead::ItemId::TomatoSeed};
    ...
    if (restoredCrops.Count() != 0 || gold!=40) return 8;
    return 0;
}
