...
int main(int argumentCount, char** arguments) {
    ...
    if (!Homestead::DecodeSave(bytes.data(), bytes.size(), decoded) || decoded.day != 4 ||
        ...
        decoded.gold != 87 || decoded.inventory[2].count != 9 ||
        decoded.tileDeltas.size() != 1 || decoded.crops.size() != 1 || decoded.crops[0].stage != 2) return 2;
    const auto version2 = Version2Save(bytes); Homestead::SaveSnapshot migratedV2;
    if (!Homestead::DecodeSave(version2.data(), version2.size(), migratedV2) ||
        migratedV2.mapId != Homestead::MapId::Farm || migratedV2.gold != 87) return 3;

    auto damaged = bytes; damaged[0] = 'X'; if (!Rejects(damaged)) return 4;
    damaged = bytes; SetU16(damaged, 4, 4); if (!Rejects(damaged)) return 4;
    ...
    damaged = bytes; damaged[36] = 0xFF; RepairChecksum(damaged); if (!Rejects(damaged)) return 8;
    damaged = bytes; SetU16(damaged, 32, 5000); RepairChecksum(damaged); if (!Rejects(damaged)) return 9;
    damaged = bytes; damaged[70] = 0x80; RepairChecksum(damaged); if (!Rejects(damaged)) return 10;
    damaged = bytes; damaged[73] = 0xFF; RepairChecksum(damaged); if (!Rejects(damaged)) return 11;
